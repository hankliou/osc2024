...
void traverse_device_tree(void *dtb_ptr, dtb_callback callback) {
    ...
    char *dt_struct_ptr = (char *)((char *)header + uint32_endian_big2little(
                                                        header->off_dt_struct));
    char *dt_strings_ptr =
        (char *)((char *)header +
                 uint32_endian_big2little(header->off_dt_strings));
    ...
    while (pointer < end) {
        ...
        // if current node is property node
            ...
            uint32_t len =
                uint32_endian_big2little(*(uint32_t *)pointer); // get len
            pointer += 4; // mov to begining
            char *name = (char *)dt_strings_ptr +
                         uint32_endian_big2little(
                             *(uint32_t *)pointer); // get ptr to name
            pointer += 4;
...
void dtb_callback_show_tree(uint32_t node_type, char *name, void *value,
                            uint32_t name_size) {
...
void dtb_callback_initramfs(uint32_t node_type, char *name, void *value,
    ...
    if (node_type == FDT_PROP && strcmp(name, "linux,initrd-start") == 0) {
        ...
        CPIO_DEFAULT_PLACE =
            (void *)(unsigned long long)uint32_endian_big2little(
                *(uint32_t *)value);
    }
}